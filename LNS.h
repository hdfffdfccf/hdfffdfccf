...
class LNS : public Solver {
  ...
  public:
    ...
    void initialize(const Problem &problem, const Solution &solution) {
        this->problem = &problem;
        shortest = make_empty_solution();
    
        vector<int> agent_order(config.num_agents);
        std::iota(agent_order.begin(), agent_order.end(), 0);
        initial_solver.solve(problem, *shortest, agent_order, false, false); // get the shortest paths
    ...
    void generate_agent_neighborhood(Subset &subset, int agent = -1);
